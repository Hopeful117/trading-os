...
public final class KrakenBrokerProvider implements BrokerProvider {
    ...
    private final KrakenPositionManagementCapability positionManagement;
    private final KrakenRiskSnapshotCapability riskSnapshots;
    public KrakenBrokerProvider(KrakenCapabilities capabilities, KrakenPositionManagementCapability positionManagement, KrakenRiskSnapshotCapability riskSnapshots) {
        this.capabilities = capabilities; this.positionManagement = positionManagement; this.riskSnapshots = riskSnapshots;
    }
    public BrokerProviderId id() { return BrokerProviderId.KRAKEN; }
    public <T> java.util.Optional<T> capability(Class<T> type) {
        if (type.isInstance(capabilities)) return java.util.Optional.of(type.cast(capabilities));
        if (type.isInstance(positionManagement)) return java.util.Optional.of(type.cast(positionManagement));
        if (type.isInstance(riskSnapshots)) return java.util.Optional.of(type.cast(riskSnapshots));
