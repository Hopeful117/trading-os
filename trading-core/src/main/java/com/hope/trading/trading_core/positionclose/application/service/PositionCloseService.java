...
public class PositionCloseService {
    ...
    private final PositionCloseLifecycleService lifecycle;

    public PositionCloseService(PositionCloseCommandRepositoryPort repository, BrokerPositionClosePort brokerPort, PositionCloseLifecycleService lifecycle) {
        this.repository = repository; this.brokerPort = brokerPort; this.lifecycle = lifecycle;
    }
    ...
    public PositionCloseCommand close(UUID userId, UUID accountId, String brokerPositionReference, String idempotencyKey) {
        ...
        PositionCloseCommand command = lifecycle.create(UUID.randomUUID(), accountId, resolveResponse.brokerAccountId(),
        ...
        try {
            command = lifecycle.transitionToSubmitted(command, Instant.now());
            command = repository.save(command);
            ...
            command = lifecycle.transitionToUnknown(command, e.getClass().getSimpleName().replace("Exception", "").toUpperCase(), Instant.now());
    ...
    public PositionCloseCommand reconcile(UUID userId, UUID commandId) {
        ...
        command = lifecycle.reconcile(command, result, Instant.now());
        return repository.save(command);
    }
    ...
    private PositionCloseCommand mapBrokerResult(PositionCloseCommand command, BrokerPositionClosePort.BrokerCloseResponse response, Instant now) {
        return switch (response.outcome()) {
            case "ACKNOWLEDGED" -> lifecycle.transitionToAcknowledged(command, response.externalOrderId(), now);
            case "REJECTED" -> lifecycle.transitionToRejected(command, response.reasonCode(), now);
            case "UNKNOWN" -> lifecycle.transitionToUnknown(command, response.reasonCode(), now);
            default -> lifecycle.transitionToUnknown(command, "UNEXPECTED_OUTCOME", now);
        };
    }
