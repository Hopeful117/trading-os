...
describe('Dashboard', () => {
  ...
  const accounts: Account[] = [
    {
      accountId: 'account-1', name: 'Primary', baseCurrency: 'USD',
      balances: { balances: { USD: 1000 } }, equity: 1000, peakEquity: 1100,
      rulesId: 'rules-1', userId: 'user-1',
    },
    {
      accountId: 'account-2', name: 'Secondary', baseCurrency: 'USD',
      balances: { balances: { USD: 500 } }, equity: 500, peakEquity: 500,
      rulesId: 'rules-2', userId: 'user-1',
    },
  ];
  ...
  it('displays alerts and degraded status', async () => {
    ...
    data.alerts = [{
      code: 'MISSING_STOP_LOSS', severity: 'WARNING',
      title: 'Position non protégée', message: 'Aucun stop loss',
      marketId: null, positionId: 'p1', occurredAt: new Date().toISOString(),
    }];
    dashboardService.findDashboard.mockReturnValue(of(data));
    await create();
  ...
  it('displays null fields as unavailable and renders risk rules', async () => {
    ...
    data.risk.rules = [{
      code: 'MAX_DAILY_LOSS', label: 'Perte max/jour',
      limit: 5, currentValue: 3.2, status: 'WARNING',
    }];
    dashboardService.findDashboard.mockReturnValue(of(data));
    await create();
    const t = text();
  ...
  it('shows risk rules when status is not UNAVAILABLE', async () => {
    ...
    data.risk.rules = [{
      code: 'MAX_DAILY_LOSS', label: 'Perte max/jour',
      limit: 5, currentValue: 1.5, status: 'SAFE',
    }];
    dashboardService.findDashboard.mockReturnValue(of(data));
    await create();
  ...
  function summary(): DashboardSummary {
    ...
    return {
      account: {
        accountId: 'account-1', accountName: 'Primary', broker: 'KRAKEN',
        currency: 'USD', balance: 1000, equity: 1020, dailyPnl: 20,
        dailyPnlPercentage: 2, currentDrawdown: 80, currentDrawdownPercentage: 7.27,
        equitySource: 'CALCULATED',
      },
      risk: {
        status: 'SAFE', usedRiskAmount: 10, usedRiskPercentage: 1,
        remainingRiskAmount: 10, remainingRiskPercentage: 1,
        dailyLossAmount: 0, dailyLossPercentage: 0, maximumDailyLossPercentage: 5,
        totalDrawdownAmount: 80, totalDrawdownPercentage: 7.27,
        maximumDrawdownPercentage: 10, rules: [],
      },
      openPositions: [], alerts: [], watchedMarkets: [],
      freshness: {
        status: 'LIVE', brokerDataAt: now, marketDataAt: now,
        calculatedAt: now, brokerDataStale: false, marketDataStale: false, warnings: [],
      },
      generatedAt: now,
    };
  }
  ...
  function pos(id: string, symbol: string, pnl: number) {
    const now = new Date().toISOString();
    return {
      positionId: id, accountId: 'account-1', marketId: 'market-1', symbol,
      side: 'BUY' as const, quantity: 1, entryPrice: 100, currentPrice: 100 + pnl,
      stopLoss: null, takeProfit: null, unrealizedPnl: pnl,
      unrealizedPnlPercentage: pnl, brokerUnrealizedPnl: pnl,
      riskAmount: 0, riskPercentage: 0, exposure: 100 + pnl,
      protectionStatus: 'MISSING_STOP_LOSS' as const, marketTradable: true,
      openedAt: now, priceOccurredAt: now, calculatedAt: now,
    };
  }
  ...
  function makeOpportunity(id: string, instrument: string): OpportunityResponse {
    const now = new Date().toISOString();
    return {
      id, version: 1, status: 'ACTIVE', instrument,
      direction: 'LONG', scenario: 'test', timeframe: '1h',
      type: 'INTRADAY', origin: 'ACTIVE_SCAN', score: 0.8,
      explanation: 'test', observationIds: [], aiAnalysisIds: [],
      evaluatedAt: now, validFrom: now, validUntil: null,
      createdAt: now, strategyMatchId: null,
    };
  }
});
